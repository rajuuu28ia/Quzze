...
import { admins, quizSettings, questions, participants } from "../shared/schema";
...
app.get("/api/quiz/public", async (req, res) => {
  try {
    ...
    const completedParticipants = await db.select().from(participants)
      .where(eq(participants.quizSettingsId, settings.id));
    const completedCount = completedParticipants.filter(p => p.completedAt !== null).length;
    
    if (completedCount >= settings.maxParticipants) {
      return res.json({ 
        available: false, 
        tooLate: true,
  ...
  try {
    ...
    if (completedCount >= settings.maxParticipants) {
      return res.status(400).json({ 
        tooLate: true,
        message: "Yah, kamu terlambat! Slot sudah penuh." 
      });
    }
